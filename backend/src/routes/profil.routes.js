const express = require("express")
const db = require("../config/db")

const router = express.Router()

router.get("/", async (req, res) => {
    try {
        const userId = req.userId

        // USER
        const [users] = await db.execute(
            "SELECT id, email, username, bio, avatar_url, xp, top3_count FROM users WHERE id = ?",
            [userId]
        )

        if (users.length === 0) {
            return res.status(404).json({ message: "User not found" })
        }

        const user = users[0]

        // STATS (parallel pour perf)
        const [
            [[{ tasksCompleted }]],
            [[{ themesCount }]],
            [[{ communityPosts }]],
            [[{ activityDays }]]
        ] = await Promise.all([
            db.execute("SELECT COUNT(*) as tasksCompleted FROM task_completions WHERE user_id = ?", [userId]),
            db.execute("SELECT COUNT(*) as themesCount FROM themes WHERE user_id = ?", [userId]),
            db.execute("SELECT COUNT(*) as communityPosts FROM posts WHERE user_id = ?", [userId]),
            db.execute(`
                SELECT COUNT(DISTINCT DATE(completed_at)) as activityDays 
                FROM task_completions 
                WHERE user_id = ?
            `, [userId])
        ])

        // STREAK FIX (important → DATE())
        const [streakRows] = await db.execute(`
            SELECT COUNT(*) as streak
            FROM (
                SELECT d,
                       DATE_SUB(d, INTERVAL ROW_NUMBER() OVER (ORDER BY d) DAY) as grp
                FROM (
                    SELECT DISTINCT DATE(completed_at) as d
                    FROM task_completions
                    WHERE user_id = ?
                ) t
            ) grouped
            GROUP BY grp
            ORDER BY MAX(d) DESC
            LIMIT 1
        `, [userId])

        const streak = streakRows.length ? streakRows[0].streak : 0

        // ACHIEVEMENTS
        const [achievements] = await db.execute(`
            SELECT 
                a.id, a.name, a.description, a.goal, a.level, a.type,
                COALESCE(ua.progress, 0) as progress,
                COALESCE(ua.completed, FALSE) as completed
            FROM achievements a
            LEFT JOIN user_achievements ua 
                ON ua.achievement_id = a.id AND ua.user_id = ?
            ORDER BY a.level ASC
        `, [userId])

        // XP / DIVISION
        const xp = user.xp ?? 0

        const divisions = [
            { name: "Fer", min: 0 },
            { name: "Bronze", min: 500 },
            { name: "Argent", min: 1500 },
            { name: "Or", min: 3000 },
            { name: "Platine", min: 6000 },
            { name: "Émeraude", min: 10000 },
            { name: "Diamant", min: 15000 },
            { name: "Maître", min: 22000 },
            { name: "Grand Maître", min: 30000 },
            { name: "Challenger", min: 40000 },
        ]

        const currentDiv = [...divisions].reverse().find(d => xp >= d.min) || divisions[0]
        const nextDiv = divisions[divisions.indexOf(currentDiv) + 1]

        const divRange = nextDiv ? nextDiv.min - currentDiv.min : 10000
        const progress = (xp - currentDiv.min) / divRange

        let tier = "I"
        if (progress >= 0.75) tier = "IV"
        else if (progress >= 0.50) tier = "III"
        else if (progress >= 0.25) tier = "II"

        const noTier = ["Maître", "Grand Maître", "Challenger"]
        const division = noTier.includes(currentDiv.name)
            ? currentDiv.name
            : `${currentDiv.name} ${tier}`

        // RESPONSE
        res.json({
            ...user,
            stats: {
                tasksCompleted,
                themesCreated: themesCount,
                communityPosts,
                activityDays,
                xp,
                division,
                top3Count: user.top3_count ?? 0,
                streak
            },
            achievements
        })

    } catch (err) {
        console.error("PROFIL ERROR:", err)
        res.status(500).json({ message: "Server error" })
    }
})

router.put("/", async (req, res) => {
    try {
        const { username, bio } = req.body

        await db.execute(
            "UPDATE users SET username = ?, bio = ? WHERE id = ?",
            [username || null, bio || null, req.userId]
        )

        const [rows] = await db.execute(
            "SELECT id, email, username, bio, avatar_url FROM users WHERE id = ?",
            [req.userId]
        )

        res.json(rows[0])

    } catch (err) {
        console.error("UPDATE PROFIL ERROR:", err)
        res.status(500).json({ message: "Server error" })
    }
})

module.exports = router